...
feeds.yaml（監視ソース定義）
    │
    ▼
inforadar.py ── RSS取得 → state.json と突合 → 新着抽出・キーワード判定
    │
...
> **注意**: 初期の URL 群は開発環境から疎通未確認です。初回の Actions 実行レポートの「⚠️ 取得失敗」を見て、動かないソースを修正してください。RSS 非提供サイト（グッドデザイン賞・JR各社プレスリリース等）の HTML 監視は今後の拡張候補です。

## 今後の拡張候補

- HTML ページの差分監視（RSS 非提供サイト向け）
