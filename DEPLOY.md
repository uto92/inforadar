...
**同一イベントを複数端末で「読み取る」にはソルトの共有が要る。**
ハッシュのソルトはイベント作成端末にしかなく、サーバにも送っていない。
そのため他端末では、そのイベントの**集計は見えるが読み取りはできない**。
受付が1台なら問題にならない。複数台で読み取る場合は招待コード方式の実装が必要
（未実装。サーバにソルトを置く案は、12桁が総当たりで探索可能になるため採らない）。
