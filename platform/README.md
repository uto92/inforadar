...
iOS プローブ（`ios-probe/0.1.0`）が読んだ実カード1枚・20ブロックで検証済み。
**詳細は [docs/real-device-verification.md](docs/real-device-verification.md)。**

要点だけ:

- **v1 の解釈は成立している。** 20ブロックすべて解釈でき、残額と連番の整合を検算した
  14区間すべてで、差分が実際の運賃・物販・チャージとして妥当な額になった。
- **連番には欠番が出る**（カードの履歴枠に残らない取引がある）。
  `amount_estimated` を連続区間に限定した設計はこの挙動に対応しており、
  実データでも欠番の前後は正しく `null` になった。
- **v2 で決め直す点が3つ**（物販レコードの時刻を保存するか / `area_code` の対応付け /
  駅として解釈しない処理種別の範囲）。いずれも生HEXが残っているので後から遡及できる。

検証レポートには検算の根拠としてデコード結果を載せているが、生HEXと `card_pseudonym` は
含めていない（一次情報は D1 の `raw_observations` にある）。
