...
7. **読取ライブラリの差し替え耐性**: 判定ロジックは `src/lib/normalize.ts` に隔離してあり、
   ライブラリ差し替えの影響範囲は ScanPage のみ（実際に html5-qrcode → Quagga2 の
   移行をこの範囲だけで完了できた）。読取可否は `npm run test:scan` で検証する
8. **1Dバーコード（Codabar/Code128）の読取条件**: 逆光・カード面の反射・暗所に弱い。
   屋外では日陰を作る、カードを画面の横長枠に合わせる、といった運用でカバーする
9. **読取は「読み取り」押下時のみ**: 押下後5秒間だけ読取を受け付ける。押していない間の
   検出は破棄するため、意図しないチェックインは発生しない。加えて同一コードは
   2.5秒のクールダウンがあり、同一人物の連続再読取はその間無視される（重複警告も出ない）
